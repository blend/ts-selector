...
test("assert check labels works as intended", async (t) => {
  ...
  const badLabelValues = { foo: "bar!!", "foo.com/bar": "baz" };
  t.truthy(selector.checkLabels(badLabelValues));
});

test("assert must check labels works as intended", async (t) => {
  const goodLabels = { foo: "bar", "foo.com/bar": "baz" };
  t.notThrows(() => {
    selector.mustCheckLabels(goodLabels);
  });
  const badLabels = { foo: "bar", "_foo.com/bar": "baz" };
  t.throws(() => {
    selector.mustCheckLabels(badLabels);
  });
});

test("assert must check key works on specific problems", async (t) => {
  t.throws(() => {
    selector.mustCheckKey("nospecialchars%^=@");
  });
});
...
test("assert check value works as intended", async (t) => {
  t.falsy(selector.checkValue(""), "should not error on empty values");
  t.falsy(selector.checkValue("foo"));
  t.falsy(selector.checkValue("bar_baz"));
  t.truthy(selector.checkValue("_bar_baz"));
  t.truthy(selector.checkValue("bar_baz_"));
  t.truthy(selector.checkValue("_bar_baz_"));
  t.truthy(selector.checkValue("a".repeat(selector.MaxValueLen + 1)));
});

test("assert must check value works as intended", async (t) => {
  t.notThrows(() => {
    selector.mustCheckValue("ok");
  });
  t.throws(() => {
    selector.mustCheckValue("_bar_baz");
  });
});
